# Includes a comprehensive snippet of codes to handle tasks in pandas dataframes such as reading and summarizing files, filtering and sorting data, renaming, adding, and removing columns, handling missing values, merging and concatenating dataframes, and other frequently used features in pandas.
import pandas as pd
import numpy as np
...
# when reading in tables, can specify separators, and note a column to be used as index separators can include tabs
# ("\t"), commas(","), pipes ("|"), etc.
...
df[
    df.column_z < 20].column_x.value_counts()  # value_counts of resulting Series, can also use .mean(), etc.
...
# drop missing values
df.dropna(
    inplace=True)  # drop a row if ANY values are missing, defaults to rows, but can be applied to columns with axis=1
...
# value does not have to equal a string — can be set as some calculated value like df.column_x.mode(),
# or just a number like 0
...
# if you don't specify a column to which the aggregation function should be applied, it will be
...
df.loc[:,
'column_x':'column_y']  # select a range of columns (i.e., selects all columns including first through last specified)
...
'''
# TODO: Define df1 and df2 better!
df1 = []
df2 = []
...
# merging dfs based on paired columns; columns do not need to have same name, but should match values;
...
# setting and then removing an index, resetting index can help remove hierarchical indexes while preserving the
# table in its basic structure
